...
import PlasmicInsertPanelTabGroup from "@/wab/client/plasmic/plasmic_kit_insert_panel/PlasmicInsertPanelTabGroup";
import PlasmicInsertPanelTabItem from "@/wab/client/plasmic/plasmic_kit_insert_panel/PlasmicInsertPanelTabItem";
import {
...
const rightSideContentWidth =
  rightSideWidth - 2 * rightSideHPadding - scrollbarWidth;
const sameRowGap = 5;
const compactPerRow = 3;
const compactItemWidth =
  (rightSideContentWidth - (compactPerRow - 1) * sameRowGap) / compactPerRow;
...
const shouldShowPreview = (group: AddItemGroup, item: AddItem): boolean => {
  ...
  return (
    ...
    (group.sectionKey === "insertable-templates" ||
      ...
      group.items.every((i) => !!i["previewImage"]) ||
      (item.type === "frame" &&
  ...
  const buildItems = React.useCallback(
    (query: string) => {
      ...
      const virtualRows = groupConsecBy(virtualItems, (item, i) =>
        ...
        const chunkSize = isCompactVirtualItem(group[0]) ? compactPerRow : 1;
          ...
          children: Object.entries(allFamilies).map(
            ([familyKey, groupsInFamily]) => {
              ...
              return someGroupInFamily.familyLabel ? (
                <PlasmicInsertPanelTabGroup
                  title={someGroupInFamily.familyLabel}
                >
                  {children}
                </PlasmicInsertPanelTabGroup>
              ) : (
...
export function isCompactVirtualItem(virtualItem: VirtualItem) {
  return virtualItem.type === "item" && virtualItem.item.isCompact;
}
        ...
        if (virtualItem.type === "header") {
          ...
          return (
            <li
              ...
              style={{
                width: isCompactVirtualItem(virtualItem)
                  ? compactItemWidth
                  : "100%",
                minWidth: isCompactVirtualItem(virtualItem)
                  ? compactItemWidth
                  : "100%",
                maxWidth: isCompactVirtualItem(virtualItem)
                  ? compactItemWidth
                  : "100%",
              }}
            >
            ...
            aliases.map((alias) => {
              ...
              if (resolved.startsWith("builtincc:")) {
                ...
                return {
                  ...
                  previewImage: (
                    <img
                      src={`https://plasmic-static1.s3.us-west-2.amazonaws.com/insertables/${alias}.svg`}
                      style={{
                        position: "relative",
                        width: "100%",
                        height: "100%",
